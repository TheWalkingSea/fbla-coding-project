...
def create_partners_menu():
    ...
    organization_contact_fk = dict(zip(contactNames, list(map(lambda x:x['id'], contactdata))))[answers['organizations']]

    # Uses another trick by filtering through contact_fk in the list of data and then getting the first item in the list filtered (there should only be one item)
    organization_fk = list(filter(lambda x:x['contact_fk'] == organization_contact_fk, orgdata))[0]['id']
